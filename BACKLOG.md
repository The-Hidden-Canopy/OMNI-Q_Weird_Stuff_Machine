...
|   | OQ-010-TELEOP | Damion/Claude | **Decide the fork/spoon/napkin branch with one experiment** | OQ-010 | Keyboard teleop (WASD xy / RF z / QE tilt / arrows rotate / SPACE gripper) wired over the *existing* dual-SO-101 scene; a human attempts `fork_1` by hand. **Human succeeds** → demonstrations are collectable, the imitation path (ACT, 30-60 min train) is live and the scripted gap is a control problem. **Human fails** → the blocker is physics/geometry and no learned policy can route around it; start with the torsional-friction delta instead. ~30 min, and it chooses between two branches that each cost days. Full reasoning and the source cross-check: [`docs/oq-010-external-sources-crosscheck-2026-09-13.md`](docs/oq-010-external-sources-crosscheck-2026-09-13.md) |
|   | OQ-010-BIMANUAL | Damion/Claude | **Put an object where both arms can reach it** | OQ-010 | Measured 2026-09-13: bases 0.52 m apart, ~0.386 m reach each, so a real shared workspace exists (midline band x [-0.12, 0.13], y [-0.09, 0.30]) — but **every object is single-arm-only** (cup R, plate/fork/napkin L, spoon R). HANDOFF / STABILIZE / COOPERATIVE_ROTATE (OQ-011) therefore cannot be demonstrated on any current object, and two-arm coordination is a scored rubric item (PARTIAL in OQ-004's audit). Fix: a staging pose inside the band, or a slide-to-centre step. Unlocks the flat-object manoeuvre too — the fingertip cannot straddle an 8 mm object lying flat (pads are a fair model of the hardware, see the evidence bundle), so it must be *tipped*, which needs a second arm as the stop. Evidence: [`evidence/benchmark_results/pad_friction_probe_2026-09-13/`](evidence/benchmark_results/pad_friction_probe_2026-09-13/README.md) |
|   | OQ-025 | Damion/Claude | **Contact honesty + realistic tableware models** | OQ-010 | **2026-09-14 pm: perception e2e — scene-trained detector, four-camera fusion, one-object-per-gripper guard, mid-run authority change demo, handoff recoveries (8/20 honest). am: realistic models built (rimmed plate, hollow cup, bent cutlery), sideways rim pinch, two-arm plate pick/place; 3 of 4 engine seeds set the whole table, 4th 4/5 — see the update in the doc.** 2026-09-13 end of day: pads were sinking 16-22 mm into objects and the plate 24 mm into the table (soft `solimp` width + mass-scaled default stiffness). Firm contact is now the default (measured 10/15 vs 9/15 placed, penetration halved); rigid direct-stiffness is opt-in `OMNIQ_RIGID_CONTACT=1` because the 32 mm puck plate wedge-jams in the jaw notch and launches at 2.9 m/s, and the solid cup's grasp collapses without penetration. Next: thin-lipped plate on a foot ring + edge pinch, hollow cup, bent-handle cutlery, then re-gate under rigid contact. Handoff success under real variation is **6/20** (was 10/10 at 2 mm jitter). Evidence and pickup plan: [`docs/contact-honesty-2026-09-13.md`](docs/contact-honesty-2026-09-13.md) |
| ~ | OQ-SKILL-001 | Damion/Claude | Skill contracts + registry (no RL) | OQ-010 | Contracts, promotion-gated registry, proposal-only runtime, hard supervisor, RL grasp adapter, and evidence verifier are landed and adversarially tested. **Remaining:** register the Intel deterministic controllers (`_do_pick`/top-down grasp) as executable manifests and route `GRASP` through the registry without changing behaviour. **The supervisor is a generalization, not an invention:** `expressive.py:248` `validate_proposal` remains the expressive-command template, while `skills/supervisor.py` now handles skill proposals. See [`docs/skill-runtime-architecture-2026-09-13.md`](docs/skill-runtime-architecture-2026-09-13.md) |
|   | OQ-SKILL-002 | — | Promotion status on every skill | OQ-SKILL-001 | TRAINED → SIM EVAL → DOMAIN RANDOMIZATION → SHADOW → SUPERVISED HARDWARE → VALIDATED → ACTIVE; selector may resolve **only** ACTIVE. Existing controllers are ACTIVE by definition, so a future policy has somewhere to not be. Builds on the 10-seed harness and `evidence/benchmark_results/` bundles |
|   | OQ-SKILL-003 | — | Residual control `u = u_IK + alpha*du_RL` | OQ-SKILL-002 | Blending layer with `0 <= alpha <= alpha_max`, shipped at `alpha = 0` (exact no-op) and raised only for a specifically validated skill under the supervisor. Composes with the existing four-stage pick as deterministic pre-grasp → bounded residual at contact → deterministic lift |
| ~ | OQ-SKILL-004 | — | Learning plane: execution corpus capture | OQ-SKILL-001 | Command-level governed capture is landed in [`integrations/intel/demonstrations.py`](integrations/intel/demonstrations.py): accepted/rejected transitions, before/after `WorldState`, and optional MuJoCo qpos/qvel/ctrl/time telemetry are written as exclusive, hashed JSONL. **Remaining:** motor-level LeRobot conversion, controller/action alignment, human-correction fields, and offline-only training export. OMNI never retrains live. |
...
| ~ | OQ-FLEET-003 | Gerron/GPT | Fleet failure reallocation and governed replan | OQ-FLEET-002, OQ-018 | Pure fleet fault reallocation now invalidates affected leases, marks failed resources offline, preserves lease owner/purpose, selects eligible replacements when capability/workspace context is supplied, and returns event-ready evidence. Governed engine transition/event integration and full task replan remain open. |
...
- OQ-024 is `~`: the provider-neutral voice boundary, speaker attribution, partial/final gating, authority boundary, interruption gate, and adversarial tests are present; the live Speechmatics websocket/audio transport remains open. **Update, 2026-09-13 (Damion/Claude): the transport is built** (`integrations/speechmatics/`, see the OQ-024 row). What is still literally unmeasured is the *live service*: no `SPEECHMATICS_API_KEY` was set on this box, so every latency number in the receipt so far comes from offline replay and is labelled as such rather than quoted as a Speechmatics figure. The mapping layer is where the real work was — `voice.py` validates integer-nanosecond timestamps, a strictly increasing sequence shared by partials *and* finals, and `t_end_ns > t_start_ns`, none of which the provider guarantees (it sends float seconds and legal zero-length partials).
- OQ-010 is still open: the recorded randomized trials show only limited object-specific success, so the scheduler/per-object attempt tally is evidence plumbing, not a manipulation acceptance result. **Update, 2026-09-11 (Damion/Claude):** `plate_1` moved from a fragile 1/10 randomized-seed hold rate to a verified 10/10 by wiring in the previously-shelved local-minimum escape (`OBJECT_GRASP_SEED_BIAS` in `intel_sim.py`) as a fixed per-object constant rather than the expensive per-attempt search that had blocked landing it — the fragility worry was checked against 10 `IntelSceneConfig(randomized=True)` seeds rather than assumed. `cup_1` (10/10, unaffected) remains the only other object with a real success path; `fork_1`/`spoon_1`/`napkin_1` are still fully unsolved after four independent escape mechanisms were tried this session (seed perturbation, annealed damping, approach-bearing variation, full free-DOF search) — closing that gap needs a genuinely different technique (analytical multi-solution IK, a precomputed configuration library, or a learned policy), not another cheap search variant. See `intel_sim.py`'s module docstring, "Eighth update".
- Documentation correction, 2026-09-13: the earlier `plate_1` 1/10 figure is historical pre-seed-bias evidence. The later v8/v9 bundles supersede it with 10/10 held grasps under the expanded randomized axes; placement remains 0/10 because the separate rim-grasp carry-safety bound is still unresolved. Do not quote the earlier row text as the current acceptance result.
- OQ-024 Voice API continuation, 2026-09-13: the optional `speechmatics-voice` bridge now supports provider-side Smart Turn events, explicit speaker focus, known-speaker bindings, and the existing TTS response sink. These remain provider seams; they do not grant authority or replace the core `VoiceRuntime` boundary. Live post-fix latency and multi-speaker acceptance remain unmeasured.
- **Update, 2026-09-12 (Damion/Claude): the Intel challenge hosts clarified "10 seeds" as ≥10 non-trivial environment variations (lighting, object location, prompt phrasing, object color/texture — entrant picks axes/count), and every bundle through v8 only varied one narrow axis (±3mm/±0.08rad object position/yaw).** `IntelSceneConfig` gained `color_jitter` and `light_diffuse_jitter`/`light_angle_jitter_rad` (tableware rgba and key-light intensity/angle — both verified physics-inert, never touching contype/conaffinity/friction/mass/solref/solimp), and `run_intel_table_evaluation_report` now cycles 10 distinct, pre-verified instruction phrasings (`TABLE_SETTING_PHRASINGS`) instead of the literal string "set the table" ten times. Re-ran the evaluation across all four combined axes (`evidence/benchmark_results/intel_table_eval_2026-09-12-v9/`): `cup_1`/`plate_1` both still hold 10/10 — the first time the "Eighth update" fix was checked against lighting/color variation, not just position jitter. **New finding, made visible by `plate_1` now succeeding in every trial instead of 1/10:** it holds 10/10 but places 0/10 — every `MOVE` rejected on `"unsafe carry separation"` (~0.121m measured against a `LEGACY_MAX_CARRY_OFFSET_M = 0.100` global bound). Root cause identified, not yet fixed: `plate_1` is deliberately grasped at its rim (`OBJECT_GRASP_OFFSET["plate_1"] = 0.078`), and the carry-safety check's single global bound was evidently tuned around `cup_1`'s centered grasp, not a legitimately-necessary large-object rim offset. A real follow-up candidate (make the bound per-object-aware, same pattern as `OBJECT_GRASP_OFFSET`/`OBJECT_HALF_HEIGHT`) — deliberately not attempted in the same pass as a safety-bound question deserves its own scrutiny, not a same-commit patch. See `intel_sim.py`'s module docstring, "Ninth update", and the v9 evidence README.
- **Update, 2026-09-13 (latest manipulation/reasoner handoff):** the gripper now commands its physical closed stop, reducing the fingertip gap to **3.6 mm** and confirming `cup_1` holds 10/10, but flat `fork_1`/`spoon_1`/`napkin_1` still need a real pre-grasp manoeuvre because the tracked jaw point and descent geometry remain wrong for thin objects. The earlier **21.3 mm** closed-gap note was a base-of-wedge measurement; the actual fingertip gap is **2.6 mm** (pad-friction receipt: `evidence/benchmark_results/pad_friction_probe_2026-09-13/`). No object currently lies in both arms' measured shared workspace, so handoff/stabilize demonstrations require repositioning or a deliberate push/tip manoeuvre; see the same receipt and `OQ-010-BIMANUAL`. On the planner side, legal-id constrained decoding raised r1 acceptance **0.121 -> 0.463** and usable plans **0.333 -> 0.667** without retraining, but exact match remains 0.000 and legal identifiers can still form incoherent plans; evidence: `evidence/constrained_decode_r1.json` and `reasoner/README.md`. Next developer: do not shrink the fingertip pads or treat constrained decoding as solved planning; focus on a physical pre-grasp manoeuvre and a real training/sequence-coherence improvement.
...
| ~ | OQ-024 | Gerron/GPT, Damion/Claude | Speechmatics realtime adapter | OQ-023 | Spoken instruction enters same goal parser and can modify an active task. **Transport landed 2026-09-13 (Damion/Claude):** `integrations/speechmatics/` — realtime v2 websocket (`transport.py`), pure provider→payload mapper (`mapper.py`), file/mic/replay CLI (`scripts/run_voice_transport.py`), synthetic session sample, 24 tests with no network or key. Offline replay proves the whole chain: S1's "don't use the left arm anymore" and "keep inference on-device" reach `RuntimeMutator` as committed graph mutations (`prefer_arm=right`, `keep_local`), unregistered S2 is denied. **Measured live the same day** (HyperX QuadCast → 16 kHz mono → `eu.rt.speechmatics.com`, 128 s, `evidence/voice_latency.json`): partials p50 **423 ms**, finals p50 **1066 ms**, zero adapter rejections. That first live session found two real defects that offline replay could not have: (1) the provider split one spoken sentence into **five** finals, so `nlu.parse` saw `"use your"` and `"left arm"` alone and the instruction never became a mutation — fixed by `UtteranceAggregator` (silence/speaker-change segmentation, fresh sequence on flush, empty finals treated as silence evidence); (2) `_rule_stop_using_arm` accepted only `"the left arm"`, and people say `"your left arm"` — determiner set widened in `nlu.py`. Replaying the real capture now commits `prefer_arm=right`. A **second** live session (halting speech) then showed acoustic segmentation is not sufficient in principle: the speaker paused mid-instruction, so the silences were real and ten fragments became ten claims. `omni_q.voice.IntentAccumulator` (Damion's design) now sits between segmentation and the runtime, holding an utterance that is not yet an instruction and joining it with the next — completeness decided by `intent_capability`, the same function `ingest_final` acts on. Constraints act immediately; a bare goal waits for the sentence to end; safety words (STOP/HOLD/BACKOFF) are never held; expired holds dispatch as observations rather than being discarded; authority stays inside `ingest_final` so denials remain auditable. That work found **two bugs outside the transport**: `_rule_prefer_arm` read the partial text `"don't use your left"` as a preference *for* the left arm (negation guard added — an inverted safety-relevant instruction), and `UtteranceAggregator.tick` mistook ~1.1 s of provider latency for silence (wall idle now measured from arrival). **Remaining: re-measure the receipt post-fix** (the table is pre-fix and final latency now includes the deliberate hold), plus multi-speaker diarization acceptance |
...
|   | OQ-028 | Gerron/GPT | Qualcomm X Elite inference node | OQ-027 | Real on-device inference returns structured detections/results — host-side reasoning scaffold landed 2026-09-10 (Omni Q side): `omni_reasoner.py` + `omni_planner.py` + `demo_intel_reasoner.py`, identity-gated IDA Omni body via vendored harness (`integrations/intel/vendor/omni_reference/`), `("reasoning",)` provider slot already declared; re-place onto X Elite when this lands. **Real vision + reasoning-driven planning composed together for the first time, 2026-09-11 (Damion/Claude):** `FrameObserver`+`OpenVINODetector` (real render → real inference → real zone mapping) and `OmniPlanner` had each been proven independently but never run on the same engine — composed them, found and fixed 3 real integration bugs (stale re-PICK of an already-held object burning the whole revision budget; MOVE routed to the wrong arm when the model omits `arm=`; redundant PICK/MOVE on an already-placed object once ownership releases), all mirroring gaps `RulePlanner` never has since it generates candidates fresh from `world.misplaced()`/`ownership` every call instead of carrying belief across turns. With all three fixed: real `cup_1` PICK+MOVE success end to end (real vision, real IK, real physics), then a clean labeled fallback to the deterministic planner once the (deliberately static, model not yet pretrained) mock reasoner's proposal is exhausted. Full writeup: `docs/oq-omni-vision-integration-2026-09-11.md`, evidence receipt at `evidence/benchmark_results/omni_vision_integration_2026-09-11/`. 382/382 tests green. |
|   | OQ-029 | Gerron/Kimi | Quantization/performance experiment for Qualcomm | OQ-028 | Record latency, memory and accuracy for viable deployment variants — **2-bit CPU eval done 2026-09-10** (`integrations/qualcomm/lowbit/` codec + `scripts/eval_yolo_2bit_cpu.py`, evidence `yolo_2bit_cpu_20260910/`; fp32/mxfp4/nvint2/mxfp2 arms, 3 tiers, cosine agreement; software-dequantize only, X Elite variants still blocked on OQ-028). **Born-compressed residency slider landed 2026-09-11** (`src/omni_q/residency.py` + `demo/residency_slider.py` + `tests/test_residency.py`): envelope-driven MXFP8→MXFP4→MXFP2→CORE_ONLY selection for a 6B body — the model never resides in FP32/BF16, the same body stays resident across MX tiers, CORE_ONLY evicts the reasoner (autonomy DEGRADED, novel tasks HOLD), graph-revision lineage + validated receipt bundles (`evidence/benchmark_results/residency_demo_*/`). **Independent audit finding + fix (Damion/Claude, 2026-09-11):** `evict_reasoner()` computed its CORE_ONLY decision by pretending the envelope was 0 bytes, but never updated the manager's own tracked `available_bytes` to match — a receipt taken right after a forced eviction would self-contradict (CORE_ONLY/DEGRADED next to a stale pre-eviction byte count, e.g. 8 GB), undermining the module's own "residency accounting is deliberately honest" goal. Fixed to zero `available_bytes` on eviction; regression test added (`test_evicted_reasoner_reports_zero_available_bytes_not_stale_pre_eviction_value`). Also fixed a minor doc/test inconsistency in `eval_secrets.stable_unit_float` (claimed a half-open `[0.65,1.35)` range; it's closed, `byte==255` lands exactly on 1.35). `evidence_bundle.py` reviewed separately, no issues found. 377/377 tests green. |
...
| ~ | OQ-HAND-011 | Gerron/Claude | Dance-while-working: speech style-mode → scheduler idle-slack flourishes | OQ-HAND-005, OQ-015 | spoken vocab ("fancy", "together", "opposite", "freeze", "back to work") → `style` mode; scheduler fills idle-arm slack with non-blocking IDLE_FLOURISH, never adding a wave or reordering; last STYLE wins; `minimum_time` strips flourishes. **Sim primitives done** (`2e3f0fd`): `FakeManipulator` supports the IDLE_FLOURISH ops; `intel_sim` renders them as real free-space joint gestures (`_FLOURISH_GESTURES`, no object/grasp); `Schedule.annotate(execute_flourishes=True)` emits them as runnable leaf steps. End-to-end dance run still gated on OQ-016 execution. |
