...
| x | OQ-009 | Gerron/Claude | Implement world-state representation | OQ-001, OQ-008 | Omni maintains objects, positions, orientations, ownership, goals and constraints across frames — `WorldState` has objects/ownership/constraints/goal/revision across frames; `contracts.Pose` (x/y/z/yaw + distance helpers) on `Detection.pose` (optional, serialises via `asdict`); `IntelTableWorld.state()` stamps every tracked object's live MuJoCo free-joint pose; symbolic camera path leaves it `None` |
