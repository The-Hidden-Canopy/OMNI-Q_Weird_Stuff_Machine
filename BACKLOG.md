...
|   | OQ-SKILL-001 | Damion/Claude | Skill contracts + registry (no RL) | OQ-010 | Existing deterministic controllers (`_do_pick`, top-down grasp) registered as contracted skills — digest, capability, preconditions, action envelope, workspace, termination, fallback, explicit `authority` block — and `GRASP` routed through the registry. Behaviour unchanged; the point is to fix the envelope vocabulary before anything learned depends on it. **The supervisor is a generalization, not an invention:** `expressive.py:248` `validate_proposal` already fails closed on revision staleness, allowed arms/primitives/regions, amplitude, axis bounds, time budget and contact allowance, and `validate_expressive_command` already re-checks at the provider boundary for the directive's own stated reason. See [`docs/skill-runtime-architecture-2026-09-13.md`](docs/skill-runtime-architecture-2026-09-13.md) |
...
|   | OQ-SKILL-004 | — | Learning plane: execution corpus capture | OQ-SKILL-001 | `(observation, skill requested, controller used, actions, outcome, failure, human correction, world-state delta)` captured for **offline** training only. OMNI never retrains live. Receipts already hold much of this but are shaped for evidence, not training |
